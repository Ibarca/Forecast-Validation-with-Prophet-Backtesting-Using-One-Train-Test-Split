...
This diagram brings together the main validation steps: the model is trained on the earlier historical data, then it forecasts the validation period, and finally the forecasted values are compared with the actual sales.
...
`scikit-learn` is one of the most widely used Python libraries for machine learning. In this article, I do not use it to train the forecasting model, because the forecast itself is created with Prophet. Instead, I use `scikit-learn` for the evaluation step, because it provides ready-to-use functions for common error metrics such as **MAE**, **RMSE**, and **MAPE**.

This makes the validation process cleaner and easier to reproduce. Instead of manually writing every formula in Python, I can compare the actual sales values with the forecasted values using standard metric functions.

The validation table contains the actual values from the validation period and the forecasted values generated by Prophet:
